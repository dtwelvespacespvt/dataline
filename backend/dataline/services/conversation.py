...
class ConversationService:
    ...
    async def delete_conversation(self, session: AsyncSession, conversation_id: UUID) -> None:
        try:
            ...
            logger.error("Error while deleting memory for conversation_id : {} ".format(conversation_id), e)
    ...
    async def query(
        ...
        try:
            ...
            logger.error("Cant Build memory for userId: {} for connection: {}".format(await self.auth_manager.get_user_id(), connection.id), e)
        ...
        try:
            ...
            logger.error("Error Getting Memory For user:{} connectionId:{}".format(await self.auth_manager.get_user_id(), connection.id), e)
